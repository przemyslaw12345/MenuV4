...
internal class UpdatingDatabse : IUpdatingDatabase
{
		...
		while (isWorkingSubLoop)
		{
			switch (optionSelected)
			{
				case "drink":
					...
					cafeMenu = _menuDbContext.Drinks.Find(itemNumber);
					UpdateProcessMainMenuMethod( cafeMenu);
				...
				case "meal":
					...
					itemNumber = _iDeletingFromDatabase.GetItemNumberMethod(optionSelected);
					cafeMenu = _menuDbContext.Meals.Find(itemNumber);
					UpdateProcessMainMenuMethod(cafeMenu);
					isWorkingSubLoop = false;
					isWorking = CheckIfToContinueUpdatingItemInTheMenuMethod(isWorking);
					break;
	...
	private void UpdateProcessMainMenuMethod(CafeMenu? cafeMenu)
	{
		bool isWorking = true;
		while (isWorking)
		{
			...
			isWorking = SelectingWhatToUpdateInTheMenuMethod(optionSelcted, cafeMenu);
		}
	...
	private bool SelectingWhatToUpdateInTheMenuMethod(string optionSelcted, CafeMenu? cafeMenu)
	{
		bool isWorking = true;
